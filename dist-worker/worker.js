...
var app = new Hono2();
app.use("*", cors());
var API_KEY = "cutad_98e7ba3c88fdfe5526740ed69f59fc71267f4a69";
var BASE_URL = "https://www.cutad.web.id/api/public";
...
app.get("/api/proxy/m3u8", async (c) => {
  try {
    ...
    if (!response.ok) return c.text(`Failed to proxy: ${response.statusText}`, response.status);
    ...
    if (url.includes(".m3u8")) {
      ...
      const rewriteUrl = /* @__PURE__ */ __name((rawUrl) => {
        let absoluteUrl = rawUrl;
        if (!absoluteUrl.startsWith("http")) {
          if (absoluteUrl.startsWith("/")) absoluteUrl = `${baserUrlObj.origin}${absoluteUrl}`;
          else absoluteUrl = `${basePath}${absoluteUrl}`;
        }
        return `/api/proxy/m3u8?url=${encodeURIComponent(absoluteUrl)}` + (referer ? `&referer=${encodeURIComponent(referer)}` : "") + (origin ? `&origin=${encodeURIComponent(origin)}` : "");
      }, "rewriteUrl");
...
app.get("/api/proxy/vtt", async (c) => {
  try {
    ...
    if (!url) return c.json({ error: "URL is required" }, 400);
    const response = await fetch(url);
    if (!response.ok) return c.text("Failed to fetch subtitle", response.status);
    let text = await response.text();
